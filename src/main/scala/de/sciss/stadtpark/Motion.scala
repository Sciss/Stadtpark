...
object Motion {
  def constant[S <: Sys[S]](value: Double): Motion[S#Tx] = Constant(value)

  def linrand[S <: Sys[S]](lo: Double, hi: Double)(implicit random: TxnRandom[S#Tx]): Motion[S#Tx] = LinRand(lo, hi)

  def exprand[S <: Sys[S]](lo: Double, hi: Double)(implicit random: TxnRandom[S#Tx]): Motion[S#Tx] = ExpRand(lo, hi)

  def sine[S <: Sys[S]](lo: Double, hi: Double, period: Int)(implicit tx: S#Tx): Motion[S#Tx] = {
    val id    = tx.newID()
  ...
  def walk[S <: Sys[S]](lo: Double, hi: Double, maxStep: Double)
                       (implicit random: TxnRandom[S#Tx], tx: S#Tx): Motion[S#Tx] = {
    val id      = tx.newID()
    val current = tx.newVar[Double](id, Double.NaN)
    new Walk(id, current, lo, hi, maxStep)
  }

  def linlin[S <: Sys[S]](in: Motion[S#Tx], inLo: Double, inHi: Double, outLo: Double, outHi: Double): Motion[S#Tx] =
    LinLin(in, inLo, inHi, outLo, outHi)

  def linexp[S <: Sys[S]](in: Motion[S#Tx], inLo: Double, inHi: Double, outLo: Double, outHi: Double): Motion[S#Tx] =
    LinExp(in, inLo, inHi, outLo, outHi)

  def coin[S <: Sys[S]](prob: Double, a: Motion[S#Tx], b: Motion[S#Tx])
                       (implicit random: TxnRandom[S#Tx]): Motion[S#Tx] = Coin(prob, a, b)

  private final case class Constant[S <: Sys[S]](value: Double) extends Motion[S#Tx] {
  ...
  private final class Walk[S <: Sys[S]](val id: S#ID, current: S#Var[Double], lo: Double, hi: Double, maxStep: Double)
    ...
    extends Motion[S#Tx] with Mutable.Impl[S] {
  ...
  private final case class Coin[S <: Sys[S]](prob: Double, a: Motion[S#Tx], b: Motion[S#Tx])
                                            (implicit random: TxnRandom[S#Tx])
    extends Motion[S#Tx] {
  ...
  private final case class LinLin[S <: Sys[S]](in: Motion[S#Tx], inLo: Double, inHi: Double,
  ...
  private final case class LinExp[S <: Sys[S]](in: Motion[S#Tx], inLo: Double, inHi: Double,
                                               outLo: Double, outHi: Double)
    extends Motion[S#Tx] {
